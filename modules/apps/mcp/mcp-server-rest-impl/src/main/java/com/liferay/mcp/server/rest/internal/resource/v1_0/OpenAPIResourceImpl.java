...
@Generated("")
@OpenAPIDefinition(
	info = @Info(description = "Single entry point for an AI to discover, understand, and invoke any tool exposed by Liferay. When the user asks for something you do not already know how to do in Liferay, start with `getToolSummariesPage` to search every tool-set at once for tools matching what the user wants, then `getToolSetToolSetNameTool` to fetch the tool's input schema, then `postToolSetToolSetNameToolInvoke` with an input map matching that schema to execute it. Only when that search returns nothing relevant, fall back to browsing: `getToolSetsPage` to list the tool-sets, then `getToolSetToolSetNameToolSummariesPage` to list every tool in one of them. A Java client JAR is available for use with the group ID 'com.liferay', artifact ID 'com.liferay.mcp.server.rest.client', and version '1.0.4'.", license = @License(name = "Apache 2.0", url = "http://www.apache.org/licenses/LICENSE-2.0.html"), title = "MCP", version = "v1.0")
)
@Path("/v1.0")
...
// LIFERAY-REST-BUILDER-HASH:1159317155