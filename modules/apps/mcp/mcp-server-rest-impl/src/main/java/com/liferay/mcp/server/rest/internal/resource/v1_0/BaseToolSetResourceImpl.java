...
public abstract class BaseToolSetResourceImpl implements ToolSetResource {
	...
	@io.swagger.v3.oas.annotations.Operation(
		description = "Use this only when `getToolSummariesPage` returns nothing relevant, or when you need to browse what Liferay offers. Searching with `getToolSummariesPage` is faster and does not require guessing a tool-set. Returns every tool-set exposed by Liferay, each with a curated description. Pick the tool-set whose description matches the user's intent and pass its `name` to `getToolSetToolSetNameToolSummariesPage` to see what tools it offers."
	)
...
// LIFERAY-REST-BUILDER-HASH:-786480844