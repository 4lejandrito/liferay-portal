...
<div id="<portlet:namespace />remote">
	<div class="alert alert-info">
		<liferay-ui:message arguments="<%= StringUtil.toLowerCase(LanguageUtil.get(request, liveGroup.getScopeSimpleLabel(themeDisplay))) %>" key="export-the-selected-data-to-the-x-of-a-remote-portal-or-to-another-x-in-the-same-portal" translateArguments="<%= false %>" />
	</div>
	...
	<aui:fieldset>
		...
		<aui:input disabled="<%= disableInputs %>" label='<%= LanguageUtil.format(request, "remote-x-id", liveGroup.getScopeSimpleLabel(themeDisplay)) %>' name="remoteGroupId" size="10" type="text" value='<%= MapUtil.getString(settingsMap, "targetGroupId", liveGroupTypeSettings.getProperty("remoteGroupId")) %>' />
