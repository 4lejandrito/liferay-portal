...
Invoke the `start-work` skill on the ticket, skipping the plan step since the fix is already in place, then invoke the `commit` skill. Find the owner of the changed files using `<repo-root>/.github/CODEOWNERS` and invoke the `pr` skill with it as the target repository. Override the user's title-only default and pass the body content explicitly so the pull request explains the regression.
