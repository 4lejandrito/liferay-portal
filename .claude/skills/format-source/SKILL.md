...
description: Format source files to align with Liferay's coding standards. Use when the user asks to format source, run the source formatter, or invokes /format-source.
name: format-source
...
Apply Liferay's source-formatting standards to the working tree, including both the automatic formatter and the manual rules listed below.

## Input

### Scope

A module path supplied via `${ARGUMENTS}`, or empty for repo-wide formatting:

- **Module path** — run the Gradle `formatSource` task from `<module-root>`.

- **Empty** — run `ant format-source-current-branch` from `portal-impl`. This Ant target inspects only committed files, so run it after creating the commit and amend any formatter changes into that commit.

## Expected Output

### Formatted Source

Source files (Java, JavaScript, JSON, JSP, Markdown, properties, shell, XML, YAML, and others) rewritten to satisfy both the automatic formatter and the manual rules below.

Apply in this order: run the automatic formatter, apply the manual rules, then rerun the automatic formatter to clean up any fallout from the manual edits. Fix every issue the automatic formatter reports.

Skip generated files. The automatic formatter already excludes them via `BaseSourceProcessor.hasGeneratedTag`; apply the same exclusion to manual edits. A file is generated when it contains any of these unquoted markers:
