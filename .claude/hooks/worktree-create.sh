...
function _set_debug_port {
	local file="$(_find_tomcat_dir "${BUNDLES_DIR}")/bin/setenv.sh"
...
function _set_gogo_shell_port {
	...
	local developer_file="$(_find_tomcat_dir "${BUNDLES_DIR}")/webapps/ROOT/WEB-INF/classes/portal-developer.properties"
...
function _set_tomcat_ports {
	local file="$(_find_tomcat_dir "${BUNDLES_DIR}")/conf/server.xml"
