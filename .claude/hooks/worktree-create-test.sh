...
function test_main_assigns_next_free_offset {
	load_common
	make_main_worktree
	setup_mocks
...
function test_main_checks_out_existing_branch {
	load_common
	make_main_worktree
	setup_mocks
...
function test_main_creates_full_worktree {
	load_common
	make_main_worktree
	setup_mocks
	...
	local setenv="${tomcat}/bin/setenv.sh"

	assert_file_contains "${setenv}" "${tomcat}"
...
function test_main_dereferences_symlinked_properties_files {
	load_common
	make_main_worktree
	setup_mocks
...
function test_main_provision_fresh_runs_ant_all {
	load_common
	make_main_worktree
	setup_mocks
...
function test_main_provision_skip_tomcat_does_not_launch_tomcat {
	load_common
	make_main_worktree
	setup_mocks
...
function test_main_reuses_elasticsearch7_config {
	load_common
	make_main_worktree
	setup_mocks
